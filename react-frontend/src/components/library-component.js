...
const Library = props => (
    <tr>
      ...
      <td>
        <Link to={"/timer/"+props.studymethod._id}>Start</Link> | <Link to={"/studymethod-edit/"+props.studymethod._id}>edit</Link> | <a href="#" onClick={() => { props.deleteStudyMethod(props.studymethod._id) }}>delete</a>
      </td>
    </tr>
  ) 
...
export default class LibraryList extends Component {
    ...
    deleteStudyMethod(id) {
        axios.delete('http://localhost:5000/library/'+id)
    ...
    libraryList() {
        return this.state.library.map(currentStudyMethod => {
          return <Library studymethod={currentStudyMethod} deleteStudyMethod={this.deleteStudyMethod} key={currentStudyMethod._id}/>;
        })
      }
